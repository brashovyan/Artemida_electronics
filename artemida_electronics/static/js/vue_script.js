var builder = new Vue({
    ...
    methods: {
        ...
        price() // расчёт цены
        {
            ...
            {
                this.result = "Компьютер собран! Проверьте все комплектующие и оформляйте заказ."
            }
        },
        ...
        order()
        {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    {
                        alert("Заказ успешно оформлен! Вы будете перемещены на страницу с корзиной (ранее добавленные товары удалятся) для подтверждения и оплаты покупки.")
